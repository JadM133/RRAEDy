...
if __name__ == "__main__":
    ...
    # The data (i.e. both x_train and y_train), should be of shape (C x H x W x T x N), 
    # where C is the number of channels, H is the height, W is the width, T is the 
    # number of timesteps, and N is the number of samples.
    ...
    method = "RRAE_DMD" # only affects the name of the saved file
    RR = True # set to False to remove SVD (ablation study)
    just_pass = False # set to True to remove the DMD (ablation study)

    model_cls = RRAE_CNN # this model will use the SVD and DMD since RRAEDy.config was imported above

    # Step 3: Specify the archietectures' parameters:
    latent_size = 64 # latent space dimension
    k_max = 9 # inital maximum number of features in the latent space (algorithm will reduce it during training).
    ...
    # steps as u need.
    pr = trainor.model(pre_func_inp(x_train[..., 0:1, -100:]), p=None, apply_W=trainor.W, apply_basis=trainor.basis, steps=x_train.shape[-2]-1)
    out = pre_func_out(x_train[:, :, -100:])
    print(jnp.linalg.norm(pr-out)/jnp.linalg.norm(out)*100)


    pr = trainor.model(pre_func_inp(x_test[..., 0:1, :]), p=None, apply_W=trainor.W, apply_basis=trainor.basis, steps=x_test.shape[-2]-1)
